...
def check_wandb_config_file(data_config_file):
    wandb_config = '_wandb.'.join(data_config_file.rsplit('.', 1))  # updated ip102.yaml path
...
class WandbLogger():
       ...
       """
        ...
        if isinstance(opt.resume, str):  # checks resume from artifact
            ...
            self.wandb_run = wandb.init(config=opt,
                                        ...
                                        entity=opt.entity,
                                        name=opt.name if opt.name != 'exp_Yolov5' else None,
                                        job_type=job_type,
                                        id=run_id,
    ...
    def log_dataset_artifact(self, data_file, single_cls, project, overwrite_config=False):
        ...
        overwrite_config (boolean) -- overwrites the ip102.yaml file if set to true otherwise creates a new
        ...
        if not log_val_only:
            path = (path.stem if overwrite_config else path.stem + '_wandb') + '.yaml'  # updated ip102.yaml path
            path = ROOT / 'data' / path
