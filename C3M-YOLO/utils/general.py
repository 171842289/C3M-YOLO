...
def check_dataset(data, autodownload=True):
    ...
    for k in 'train', 'val', 'names':
        assert k in data, f"ip102.yaml '{k}:' field missing ❌"
...
def yaml_load(file='ip102.yaml'):
    # Single-line safe yaml loading
    with open(file, errors='ignore') as f:
        return yaml.safe_load(f)


def yaml_save(file='ip102.yaml', data={}):
...
def download(url, dir='.', unzip=True, delete=True, curl=False, threads=1, retry=3):
    # Multithreaded file download and unzip function, used in ip102.yaml for autodownload
...
def increment_path(path, exist_ok=False, sep='', mkdir=False):
    # Increment file or directory path, i.e. runs/exp_Yolov5 --> runs/exp_Yolov5{sep}2, runs/exp_Yolov5{sep}3, ... etc.
