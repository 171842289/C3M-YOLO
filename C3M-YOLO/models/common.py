...
class Conv(nn.Module):
    # CBS模块(conv2D+BN+SiLU)，参数：(ch_in, ch_out, kernel, stride, padding, groups, dilation, activation)
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        ...
        # nn.identity()不改变input输入，直接返回input
...
#         self.bn = nn.BatchNorm2d(c2)
#         # nn.identity()不改变input输入，直接返回input
#         self.act = DyReLU(c2) if act is True else act if isinstance(act, nn.Module) else nn.Identity()
#
#     def forward(self, x):
#         return self.act(self.bn(self.conv(x)))
#
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        ...
        self.cv1 = Conv(c1, c_, 1, 1)  # 第一个1x1卷积
        self.cv2 = Conv(c_, c2, 3, 1, g=g)  # 第二个3x3卷积
        self.add = shortcut and c1 == c2  # 定义一个bool类型add，当shortcut为True且c1，c2通道数一样时add为真

    def forward(self, x):
        # 如果self.add为True就会将输入输出相加后再输出(res参差结构
...
class Focus(nn.Module):
    ...
    def forward(self, x):  # x(b,c,w,h) -> y(b,4c,w/2,h/2) 下采样效果
        # 图片被分为4块。x[..., ::2, ::2]指图片的左上角那一块， x[..., 1::2, ::2]指右上角那一块，x[..., ::2, 1::2]指左下角那一块，
        # x[..., 1::2, 1::2]指右下角那一块。都是每隔一个采样（采奇数列）。用cat连接这些采样图，生成通道数为12的特征图
...
class Detections:
    ...
    def save(self, labels=True, save_dir='runs/detect/exp_Yolov5', exist_ok=False):
        save_dir = increment_path(save_dir, exist_ok, mkdir=True)  # increment save_dir
        self._run(save=True, labels=labels, save_dir=save_dir)  # save results

    def crop(self, save=True, save_dir='runs/detect/exp_Yolov5', exist_ok=False):
...
class MBottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = MConv(c1, c_, 128, kernel_size=1, stride=1, use_se=1, use_hs=1)  # 第一个1x1卷积
        self.cv2 = MConv(c_, c2, 128, kernel_size=3, stride=1, use_se=1, use_hs=1)  # 第二个3x3卷积
        self.add = shortcut and c1 == c2  # 定义一个bool类型add，当shortcut为True且c1，c2通道数一样时add为真
...
class C3M(C3):
    # C3 module with MBottleneck
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)
        self.m = nn.Sequential(*(MBottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))
...
class ODBottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = ODConv2d(c1, c_, 1, 1)  # 第一个1x1卷积
        self.cv2 = ODConv2d(c_, c2, 3, 1, groups=g)  # 第二个3x3卷积
        self.add = shortcut and c1 == c2  # 定义一个bool类型add，当shortcut为True且c1，c2通道数一样时add为真
...
class C3OD1(C3):
    # C3 module with ODBottleneck()
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(ODBottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))


# C3CoT
class C3CoT(C3):
    # C3 module with CoTBottleneck()
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
...
def drop_path(x, drop_prob: float = 0., training: bool = False):
    ...
    output = x.div(
        keep_prob) * random_tensor  # 若要保持期望和不使用dropout时一致，就要除以p。
    # 参考网址：https://www.cnblogs.com/dan-baishucaizi/p/14703263.html
    return output
...
# DynamicDet
def adaptive_sigmoid(logits, hard=False, threshold=0.5):  # sigmoid 函数重写
...
class AdaptiveRouter(nn.Module):
    ...
    def forward(self, xs, thres=0.5):
        ...
        if self.training:
            xs = adaptive_sigmoid(xs, hard=False, threshold=thres)
        else:
            xs = xs.sigmoid()
        return xs
